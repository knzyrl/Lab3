...
public class JSONTranslationExample {
    ...
    public String getCanadaCountryNameSpanishTranslation() {
        for (int i = 0; i < jsonArray.length(); i++) {
            ...
            if (country.has("code") && "can".equalsIgnoreCase(country.getString("code"))) {
                return country.getString("es");
            }
        }
        return "Canada not found";
    }
    ...
    public String getCountryNameTranslation(String countryCode, String languageCode) {
        for (int i = 0; i < jsonArray.length(); i++) {
            JSONObject country = jsonArray.getJSONObject(i);
            if (country.has("code") && countryCode.equalsIgnoreCase(country.getString("code"))) {
